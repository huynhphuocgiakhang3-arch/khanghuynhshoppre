...
const createOrder = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const purchaseNotes = [];
    // Ghi lai tong tien cua TUNG dong san pham (theo productId) - dung rieng
    // de tinh xem ma giam gia co ap dung duoc cho nhung san pham nao trong
    // don nay khong (xem utils/couponHelper.js).
    ...
    for (const item of items) {
      ...
      if (hasVariants) {
        ...
        couponEligibleItems.push({ productId: product._id.toString(), itemTotal: finalPrice * quantity });
        ...
        // Uu tien ghi chu rieng cua goi (variant.purchaseNote), neu goi
        // khong co thi dung ghi chu chung cua san pham (product.purchaseNote).
        const variantNote = (variant.purchaseNote || product.purchaseNote || '').trim();
        if (variantNote) {
          purchaseNotes.push(`${product.name} (${variant.name}): ${variantNote}`);
        }
        ...
        if ((product.purchaseNote || '').trim()) {
          purchaseNotes.push(`${product.name}: ${product.purchaseNote.trim()}`);
        }
    ...
    // Ap dung ma giam gia (neu khach co nhap) - tinh TOAN BO lai o server
    // (dung ham dung chung voi route validate cong khai), KHONG BAO GIO tin
    // so tien giam do client tu gui len, tranh bi gia mao/sua request.
    let couponResult = { coupon: null, discountAmount: 0, error: null };
    if (couponCode && String(couponCode).trim()) {
      couponResult = await calculateCouponDiscount(couponCode, couponEligibleItems, session);
      if (couponResult.error) {
        throw new AppError(couponResult.error, 400);
      }
    ...
    // Neu ma giam gia hop le duoc dung, tang bo dem thong ke usedCount NGAY
    // TRONG CUNG transaction voi don hang - dam bao neu don hang loi/rollback
    // thi so lan dung cung tu dong rollback theo, khong bi dem "ao".
    if (couponResult.coupon) {
      await Coupon.updateOne({ _id: couponResult.coupon._id }, { $inc: { usedCount: 1 } }, { session });
    }
