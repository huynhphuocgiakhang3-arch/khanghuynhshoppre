...
let isFetchingUpdates = false; // chan tuyet doi 2 vong getUpdates chay song song
...
/**
 * Xoa webhook (neu co) truoc khi bat dau long-polling. Day la nguyen nhan
 * PHO BIEN NHAT khien bot "thinh thoang bam lenh rat lau moi phan hoi":
 * neu bot token nay TUNG duoc cau hinh webhook (vd deploy thu/test truoc
 * do), Telegram se tra loi getUpdates bang loi 409 Conflict LIEN TUC cho
 * toi khi webhook duoc xoa - moi lan gap loi nay vong lap phai cho 5s roi
 * thu lai (xem startTelegramBotPolling), cong don lai tao cam giac bot
 * "dong" rat lau. Goi 1 lan luc khoi dong la du, an toan du truoc do KHONG
 * co webhook nao (Telegram tra ve thanh cong, khong lam gi ca).
 */
const clearWebhookIfAny = async () => {
  const token = process.env.TELEGRAM_BOT_TOKEN;
  if (!token) return;
  try {
    ...
    console.error('[telegram-bot] Khong the xoa webhook cu (bo qua, van tiep tuc polling):', err.response?.data || err.message);
  }
};
...
const pollOnce = async () => {
  ...
  // Bao ve chong 2 vong poll chong len nhau - Telegram CHI cho phep 1 ket
  // noi getUpdates dang mo tai 1 thoi diem cho cung 1 bot token, goi chong
  // se bi 409 Conflict va lam CHAM luon ca vong dang cho hop le.
  if (isFetchingUpdates) return;
  isFetchingUpdates = true;

  let data;
  try {
    // offset: chi lay tin nhan MOI hon lan truoc. timeout=25 (giay) la tham
    // so long-polling cua Telegram (giu ket noi mo toi 25s cho co tin nhan
    // moi). Client timeout (axios) PHAI LON HON han muc nay - neu khong
    // axios se tu huy request dung luc Telegram sap tra loi, gay ra dung
    // trieu chung "thinh thoang phai doi rat lau" vi request bi huy nua
    // chung roi phai cho den vong lap sau moi thu lai duoc.
    const res = await axios.get(`https://api.telegram.org/bot${token}/getUpdates`, {
      params: { offset: pollingOffset, timeout: 25, allowed_updates: JSON.stringify(['message', 'callback_query']) },
      timeout: 45000,
    });
    data = res.data;
  } finally {
  ...
  const loop = async () => {
    try {
      ...
      const reason = isConflict
        ? 'co 1 tien trinh KHAC cung dang goi getUpdates cho cung bot token nay (Conflict 409) - kiem tra xem co dang chay 2 instance server cung luc khong'
        : err.response?.data?.description || err.message;

      // Backoff tang dan (toi da 20s) thay vi luon co dinh 5s - tranh spam
      // request lien tuc khi loi keo dai (vd mat mang tam thoi), nhung van
      // hoi phuc nhanh (bat dau tu 3s) khi chi la loi thoang qua.
      const delayMs = Math.min(3000 * consecutiveErrors, 20000);
      console.error(`[telegram-bot] Loi polling (${reason}), thu lai sau ${Math.round(delayMs / 1000)}s.`);
      setTimeout(loop, delayMs);
    }
  };

  // Xoa webhook cu (neu co) TRUOC khi bat dau vong lap dau tien - xem giai
  // thich chi tiet o clearWebhookIfAny().
  clearWebhookIfAny().finally(() => {
    registerBotCommands();
    loop();
  });
