...
export default function ProductDetailPage() {
  ...
  const [appliedCoupon, setAppliedCoupon] = useState(null); // { code, name, discountAmount }
  ...
  // Doi goi (variant) khien gia thay doi -> so tien duoc giam cua ma coupon
  // (neu dang ap dung) co the khong con dung nua, bo ap dung de tranh hien
  // sai so tien, khach can bam "Ap dung" lai.
  useEffect(() => {
  ...
  // Gia VIP (da duoc server tinh san va tra ve trong field vipPrice neu
  // khach dang nhap la VIP guest va san pham du dieu kien giam - xem
  // product.controller.js decorateVipPricing)
  const displayVipPrice = hasVariants ? selectedVariant?.vipPrice ?? null : product.vipPrice ?? null;
        ...
        <div className="mt-8 grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:sticky lg:top-24 lg:self-start">
            <div className="rounded-2xl border border-ink-700 bg-ink-800/60 p-6">
              ...
              {displayVipPrice ? (
                <div>
                  <span className="inline-flex items-center gap-1 text-xs font-bold text-amber-300 mb-1">
                    👑 GIÁ VIP GUEST -50%
                  </span>
                  <div className="flex items-baseline gap-3">
                    <span className="text-mist-500 line-through text-sm">{formatVND(displayPrice)}</span>
                    <span className="font-display font-extrabold text-3xl text-amber-300">{formatVND(displayVipPrice)}</span>
                  </div>
                </div>
              ) : (
                <div className="flex items-baseline gap-3">
              ...
              {/* O nhap ma giam gia - ap dung truoc, thay ngay so tien duoc giam */}
              {!isOutOfStock && (
                <CouponInput
                  productId={product._id}
                  quantity={1}
                  variantName={hasVariants ? selectedVariantName : undefined}
              ...
              {appliedCoupon && (
                <div className="mt-2 flex items-center justify-between text-sm">
                  <span className="text-mist-400">Thành tiền sau giảm</span>
                  <span className="font-display font-bold text-lg text-green-400">
                    {formatVND(Math.max(0, (displayVipPrice || displayPrice) - appliedCoupon.discountAmount))}
                  </span>
                </div>
              )}
