...
import { formatVND, formatDateTime, ORDER_STATUS_MAP, parseDeliveredLink, getSafeDownloadFilename, downloadFileAsBlob } from '../../lib/utils';
...
function OrdersContent() {
  ...
  const handleCopyLink = (link) => {
    ...
    const { url } = parseDeliveredLink(link);
    navigator.clipboard.writeText(url);
    toast.success('Đã sao chép link!');
  };

  const handleDownload = async (link, key) => {
    const { label, url } = parseDeliveredLink(link);
    const filename = getSafeDownloadFilename(url, label);
    setDownloadingKey(key);
    try {
      await downloadFileAsBlob(url, filename);
    } catch (err) {
      // Fallback: neu fetch bi loi (vd mang, CORS), mo thang URL goc o tab moi
      // de nguoi dung van tai duoc file thay vi khong lam gi ca.
      toast.error('Không tải trực tiếp được, đang mở file gốc...');
          ...
          orders.map((order) => {
            ...
            return (
              <div key={order._id} className="rounded-2xl border border-ink-700 bg-ink-800/40 p-5">
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-1.5 shrink-0">
                              ...
                              <button
                                onClick={() => handleDownload(link, key)}
                                disabled={isDownloading}
                                className="p-1.5 rounded-md hover:bg-ink-700 text-ember-400 disabled:opacity-50"
                                title="Tải xuống"
                              >
                                <FiDownload size={13} className={isDownloading ? 'animate-pulse' : ''} />
                              </button>
                            </div>
                          </div>
                        );
                ...
                {/* Ghi chu san pham sau khi mua (huong dan su dung, bao hanh...) */}
                {order.status === 'paid' && order.purchaseNotes && order.purchaseNotes.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-ink-700">
                    <p className="text-xs font-medium text-gold-500 mb-2 flex items-center gap-1.5">
                      📝 Ghi chú sử dụng
                    </p>
                    <div className="space-y-1.5">
                      {order.purchaseNotes.map((note, idx) => (
                        <p key={idx} className="rounded-lg bg-ink-900/60 px-3 py-2 text-xs text-mist-300 whitespace-pre-line">
                          {note}
                        </p>
                      ))}
                    </div>
                ...
                <div className="mt-3 pt-3 border-t border-ink-700 flex justify-between items-center">
                  ...
                  <div className="text-right">
                    {order.vipDiscountAmount > 0 && (
                      <p className="text-[11px] text-amber-300 font-medium">👑 Tiết kiệm {formatVND(order.vipDiscountAmount)}</p>
                    )}
                    {order.couponDiscountAmount > 0 && (
                      <p className="text-[11px] text-green-400 font-medium">
                        🏷️ Mã {order.couponCode}: -{formatVND(order.couponDiscountAmount)}
                      </p>
                    )}
