...
import { FiPlus, FiEdit2, FiTrash2, FiVideo, FiStar } from 'react-icons/fi';
...
export default function AdminProductsPage() {
  ...
  const handlePin = async (product) => {
    try {
      await api.patch(`/admin/products/${product._id}/pin`);
      toast.success(product.isPinned ? 'Đã bỏ ghim.' : 'Đã ghim sản phẩm lên đầu.');
      fetchProducts();
    } catch (error) {
                  ...
                  <tr key={product._id} className="border-b border-ink-700/50">
                    <td className="py-3 px-4 text-mist-100 font-medium max-w-[220px] truncate">
                      <span className="inline-flex items-center gap-1.5">
                        {product.isPinned && (
                          <FiStar className="text-gold-400 shrink-0 fill-gold-400" size={13} title="Đã ghim" />
                        )}
                        {product.name}
                    ...
                    <td className="py-3 px-4">
                      <div className="flex gap-2">
                        <button
                          ...
                          className={`p-2 rounded-lg hover:bg-ink-700 ${product.isPinned ? 'text-gold-400' : 'text-mist-400'}`}
                          title={product.isPinned ? 'Bỏ ghim' : 'Ghim lên đầu'}
                        >
                          <FiStar size={16} className={product.isPinned ? 'fill-gold-400' : ''} />
                        </button>
                        <button
                          onClick={() => {
