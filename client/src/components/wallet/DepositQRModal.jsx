...
export default function DepositQRModal({ onClose, onSuccess }) {
  ...
  const startPolling = (transactionId) => {
    pollRef.current = setInterval(async () => {
      try {
        const { data } = await api.get(`/wallet/deposit/${transactionId}/status`);
        if (data.data.status === 'success') {
          ...
          clearInterval(pollRef.current);
          toast.error('Giao dịch thất bại. Vui lòng thử lại hoặc liên hệ hỗ trợ.');
        }
      } catch (error) {
      ...
      <div className="glass-panel rounded-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
        {step === 'input' && (
          <>
            ...
            <p className="text-sm text-mist-400 mt-1 mb-5">Chọn hoặc nhập số tiền muốn nạp</p>
